...
int add_aggr_field(Command_t *cmd, const char *argument) {
    size_t field_len = cmd->cmd_args.sel_args.field_len;
    char **buf = (char**)malloc(sizeof(char*) * (field_len+1));
    if (buf == NULL) {
        return 0;
    }

    if (cmd->cmd_args.sel_args.field) {
        memcpy(buf, cmd->cmd_args.sel_args.field, sizeof(char*) * field_len);
        free(cmd->cmd_args.sel_args.field);
    }

    cmd->cmd_args.sel_args.field = buf;
    cmd->cmd_args.sel_args.field[field_len] = strdup(argument);
    cmd->cmd_args.sel_args.field_len++;
